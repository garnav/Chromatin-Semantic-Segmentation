...
from keras.layers import Conv1D, MaxPooling1D, UpSampling1D, Activation, Reshape, Input, Permute, BatchNormalization
...
def create_seg_net(num_classes, input_shape, init_kernel_size, mid_kernel_size, init_num_filters):
    ...
    model.add(Conv1D(init_num_filters * 8, mid_kernel_size, border_mode='same', name="conv3"))
    model.add(BatchNormalization())
    model.add(Activation('relu'))
